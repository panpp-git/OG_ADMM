...
class OGFreq_bn(torch.nn.Module):
    def __init__(self, LayerNo, signal_dim, n_filters, inner, device, fr_size):
        ...
        self.A = torch.nn.Parameter(torch.randn((signal_dim, fr_size)) / self.const + 1j * torch.randn(
            (signal_dim, fr_size)) / self.const).type(torch.complex64)
        self.AH = torch.nn.Parameter(torch.randn((fr_size, signal_dim)) / self.const + 1j * torch.randn(
            (fr_size, signal_dim)) / self.const).type(torch.complex64)
        self.B = torch.nn.Parameter(torch.randn((signal_dim, fr_size)) / self.const + 1j * torch.randn(
            (signal_dim, fr_size)) / self.const).type(torch.complex64)
        self.BH = torch.nn.Parameter(torch.randn((fr_size, signal_dim)) / self.const + 1j * torch.randn(
            (fr_size, signal_dim)) / self.const).type(torch.complex64)
    ...
    def forward(self, y):
        ...
        x_err_layers = []  # for computing symmetric loss
        beta_err_layers = []

        for i in range(self.LayerNo):
            [x, beta, alpha, x_err, beta_err] = self.fcs[i](x, beta, y, alpha, self.Aori, self.AoriH, self.Agrad,
                                                            self.AgradH)
...
class OGFreq_bn_withmid(torch.nn.Module):
    def __init__(self, LayerNo, signal_dim, n_filters, inner, device, fr_size):
        super(OGFreq_bn_withmid, self).__init__()
        onelayer = []
        self.LayerNo = LayerNo
        self.n_filter = n_filters
        self.inner = inner
    ...
    def forward(self, y):
        ...
        beta_softthresholds = []
        for i in range(self.LayerNo):
            [x, beta, alpha, x_err, beta_err,x_softthreshold,beta_softthreshold] = self.fcs[i](x, beta, y, alpha, self.Aori, self.AoriH, self.Agrad,
                                                            self.AgradH)
            x_err_layers.append(x_err)
            beta_err_layers.append(beta_err)
            x_layers.append(x.squeeze(-1))
            beta_layers.append(beta.squeeze(-1))
            x_softthresholds.append(x_softthreshold)
            beta_softthresholds.append(beta_softthreshold)
        x_final = x.squeeze(-1)
        beta_final = beta.squeeze(-1)

        return [x_layers, x_err_layers, beta_layers, beta_err_layers,x_softthresholds,beta_softthresholds]
