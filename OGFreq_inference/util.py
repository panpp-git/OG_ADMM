...
import torch
import errno
from OGFreq_bn import OGFreq_bn,OGFreq_bn_withmid
from OGFreq import OGFreq
...
def load(fn, module_type, device = torch.device('cpu')):
    ...
    if device == torch.device('cpu'):
        args.use_cuda = False
    if module_type == 'og_bn':
        model = OGFreq_bn(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    elif module_type == 'og_mid':
        model = OGFreq_bn_withmid(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    elif module_type == 'og':
    ...
    optimizer, scheduler = set_optim(args, model, 'og')
