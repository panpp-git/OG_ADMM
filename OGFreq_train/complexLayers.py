...
from complexFunctions import complex_relu, complex_max_pool2d, complex_avg_pool2d,complex_relu_beta
...
def apply_complex_mine(fr, fi, input):
    r,i=torch.chunk(input,2,1)
    ret_real=(fr(r) - fi(i))
    ret_imag=(fr(i) + fi(r))
...
class beta_ComplexReLU(Module):

    def forward(self, input):
        return complex_relu_beta(input)
